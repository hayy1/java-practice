...
SELECT
       STUDENT_NAME 이름
     , FLOOR(MONTHS_BETWEEN(ENTRANCE_DATE, TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD')) / 12) AS 나이
  FROM TB_STUDENT
...
--     작성하시오.
SELECT
       COUNT(STUDENT_NAME) AS "지도 교수를 배정받지 못한 학생의 수"
  FROM TB_STUDENT
...
SELECT
 ...
 ORDER BY EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(PROFESSOR_SSN, 1, 6), 'YYMMDD')) DESC;
